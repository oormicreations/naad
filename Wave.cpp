...
BOOL CWave::MixWithDelay(CWave& wave, UINT delay)
{
	BOOL bResult = FALSE;
	...
	{
		// Check for valid sound format
		SHORT ch = wave.GetChannels();
		DWORD srate = wave.GetSampleRate();
		SHORT bps = wave.GetBitsPerSample();
		DWORD nsamples = wave.GetSize() / (bps>>3);

		DWORD destsamples = GetSize()/ (m_Format.bitsPerSample >> 3); //will be 2x for stereo
